...
class DocWordCash extends DocWord
{
    public static function DocWordSave($id){
        ...
        $file_dir = __DIR__ . '/Helpers/template_cash.docx';
        ...
        if (file_exists($file_dir)) {
          ...
          /*  $templateProcessor->setValue('FIRM_NAME', $doc[0]->firm->name);
            ...
            $templateProcessor->setValue('DATE_ISSUE', date('d-m-Y', strtotime($doc[0]->date_issue)));
         ...
         /*   $templateProcessor->setValue('CONTRACTOR_NAME', $doc[0]->contractor->name);
            ...
            $templateProcessor->cloneRow('NUMS', count($doc_tr));

            foreach ($doc_tr as $tr) {
                $templateProcessor->setValue('NUMS#' . $tr->trnumber, $tr->trnumber);
                $templateProcessor->setValue('SERVICES#' . $tr->trnumber, $tr->service . " " . $tr->route_contractor . " " . $tr->sender);
                $templateProcessor->setValue('DIFS#' . $tr->trnumber, $tr->different);
                $templateProcessor->setValue('UNITNAMES#' . $tr->trnumber, $tr->unit->name);
                $templateProcessor->setValue('COUNTS#' . $tr->trnumber, $tr->count);
                $templateProcessor->setValue('PRICES#' . $tr->trnumber, $tr->price);
                $templateProcessor->setValue('PRICE_TRS#' . $tr->trnumber, $tr->total_price_tr);
            }
